    ...
    <div className="bg-zinc-950 text-white">
      ...
      <header className="sticky top-0 z-40 bg-zinc-950/80 backdrop-blur-lg">
        <nav className="mx-auto flex max-w-7xl items-center justify-between p-4">
          ...
          <a href="#apply" className="rounded-xl bg-primary px-4 py-2 font-semibold text-zinc-900 hover:bg-secondary">Apply&nbsp;Now</a>
        </nav>
      </header>
      ...
      <section className="relative flex min-h-screen flex-col items-center justify-center text-center">
        ...
        <p className="mt-4 max-w-md text-lg">Expert management, viral marketing and 24/7 chat support that skyrocket your OnlyFans revenue.</p>
        <a href="#apply" className="mt-8 rounded-xl bg-white px-6 py-3 font-semibold text-zinc-900 hover:bg-zinc-200">Apply&nbsp;Now</a>

        {/* payout screenshots */}
        <div className="mt-16 flex flex-col gap-6 sm:flex-row">
          <Image src="/images/payout1.png" width={220} height={120} alt="Payout screenshot" className="rounded-2xl shadow-xl backdrop-blur" priority />
          <Image src="/images/payout2.png" width={220} height={120} alt="Payout screenshot" className="rounded-2xl shadow-xl backdrop-blur" />
          <Image src="/images/payout3.png" width={220} height={120} alt="Payout screenshot" className="rounded-2xl shadow-xl backdrop-blur" />
        </div>
      </section>

      {/* ---------- SERVICES ---------- */}
      <motion.section id="services" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.6}} className="mx-auto max-w-6xl px-4 py-20">
      ...
      {/* ---------- RESULTS ---------- */}
      <motion.section id="results" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.6}} className="bg-zinc-900 py-20">
      ...
      {/* ---------- HOW IT WORKS ---------- */}
      <motion.section id="how" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.6}} className="mx-auto max-w-6xl px-4 py-20">
        ...
        <ol className="mt-12 space-y-10 lg:grid lg:grid-cols-3 lg:gap-10 lg:space-y-0">
          {[
            ...
            <li key={title} className="rounded-2xl bg-zinc-900 p-8 shadow-lg">
              <span className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-primary font-bold text-zinc-900">{i+1}</span>
      ...
      {/* ---------- TESTIMONIALS ---------- */}
      <motion.section id="testimonials" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.6}} className="bg-zinc-900 py-20">
      ...
      <section id="apply" className="mx-auto max-w-2xl px-4 py-24">
        ...
        <form action="https://formspree.io/f/your-endpoint" method="POST" className="mt-10 space-y-6">
          {["Name","Email","OnlyFans Handle"].map((label)=>(
            <div key={label}>
              <label className="block text-sm font-medium">{label}</label>
              <input name={label.toLowerCase().replace(/\s+/g,'-')} required type={label==="Email"?"email":"text"}
          ...
          <div>
            ...
            <input name="goal" type="number" min="0" className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-800 p-3 placeholder-zinc-500 focus:ring-2 focus:ring-secondary"/>
          </div>
          <button type="submit" className="w-full rounded-xl bg-primary py-3 font-semibold text-zinc-900 hover:bg-secondary">Submit Application</button>
        </form>
      </section>
      ...
      <footer className="bg-zinc-950 py-12">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-4 sm:flex-row sm:justify-between">
          ...
          <nav className="flex gap-6 text-sm">
            ...
            <a href="#apply" className="hover:text-primary">Apply</a>
          </nav>
        </div>
      </footer>
    </div>
